...
exports['plato'] = {
	...
	'test empty file': function(test) {
		...
		plato.inspect(files, null, {}, function(reports) {
			test.equal(
				reports.length,
				2,
				'Should not attempt to report on empty files'
			);
			test.done();
		});
	},
	'test file glob': function(test) {
		...
		plato.inspect(files, null, {}, function(reports) {
			test.equal(
				reports.length,
				5,
				'Should properly test against the array produced by the glob'
			);
			test.done();
		});
	},
	'test report structure': function(test) {
		...
		var files = ['test/fixtures/a.js', 'test/fixtures/b.js'];
	...
	'test overview report structure': function(test) {
		...
		plato.inspect(files, null, {}, function(reports) {
			...
			test.ok(
				overview.summary.total.jshint >= 0,
				'Should contain total jshint issues'
			);
			test.ok(overview.summary.total.sloc > 0, 'Should contain total sloc');
			test.ok(
				overview.summary.total.maintainability > 0,
				'Should contain total maintainability'
			);
			test.ok(
				overview.summary.average.jshint >= 0,
				'Should contain average jshint issues'
			);
			test.ok(overview.summary.average.sloc > 0, 'Should contain average sloc');
			test.ok(
				overview.summary.average.maintainability > 0,
				'Should contain average maintainability'
			);
			test.equal(
				overview.reports.length,
				files.length,
				'Should contain right number of reports'
			);
			test.done();
		});
	},
	'test file with shebang': function(test) {
		...
		plato.inspect(files, null, {}, function(reports) {
			test.equal(
				reports.length,
				3,
				'Should report on files starting with a shebang'
			);
			test.done();
		});
	},
	'test noempty line option': function(test) {
		...
		var files = ['test/fixtures/multipleEmptyLines.js'];

		plato.inspect(files, null, { noempty: true }, function(reports) {
			var overview = plato.getOverviewReport(reports);
			test.ok(
				overview.summary.total.sloc === 10,
				'Should contain total sloc without empty lines counted'
			);
			test.done();
		});
	}
